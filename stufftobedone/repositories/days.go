...
func (r *DayRepository) Day(bookID string, dateAsInt int) ([]domain.Day, error) {
    ...
    // filter Days where not current scheduled, is not part of a project and is not complete
    q := datastore.NewQuery("Days").Filter("DateAsInt = ", dateAsInt).Filter("IsActioned = ", false)
