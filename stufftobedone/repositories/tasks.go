...
func (r *TaskRepository) Later(bookID string) ([]domain.Task, error) {
    ...
    // filter Tasks where not current scheduled, is not part of a project and is not complete
    q := datastore.NewQuery("Tasks").Filter("CurrentDate = ", 0).Filter("ProjectID =", "").Filter("IsComplete =", false).Filter("IsDeleted =", false).Order("Created")
...
func (r *TaskRepository) Complete(bookID string) ([]domain.Task, error) {
    tasks := []domain.Task{}
    globalContext := appengine.NewContext(r.request)
    
    // filter Tasks where not current scheduled, is not part of a project and is complete
    q := datastore.NewQuery("Tasks").Filter("CurrentDate = ", 0).Filter("ProjectID =", "").Filter("IsComplete =", true).Filter("IsDeleted =", false).Order("-Completed")
...
func (r *TaskRepository) Create(elementName string, bookID string, dayAsInt int, projectID string, userID string) (domain.Task, error) {
    ...
    task := domain.Task{ ID: "", BookID: bookID, Data: "", ElementName: elementName, ProjectID: projectID, IsComplete: false, CurrentDate: dayAsInt, Created: time.Now(), CreatedBy: userID, Updated: time.Now(), UpdatedBy: userID }
